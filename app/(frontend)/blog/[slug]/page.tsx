...
type Blog = { title: string; excerpt?: string | null; content?: string | null; publishedAt?: string | null; createdAt?: string | null; featuredImage?: Media | string | number | null };

export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  if (!process.env.MONGODB_URI || !process.env.PAYLOAD_SECRET) notFound();
  const { slug } = await params;
  let blog: Blog | undefined;
  try {
    const payload = await getPayload({ config });
    const result = await payload.find({ collection: "industrial-blogs", where: { and: [{ slug: { equals: slug } }, { status: { equals: "published" } }] }, limit: 1, depth: 1 });
    blog = result.docs[0] as unknown as Blog | undefined;
  } catch { notFound(); }
  if (!blog) notFound();
